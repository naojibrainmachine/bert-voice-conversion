...
class encoder:
    def __init__(self,input_nums,hidden_nums,output_nums,mask=False,multi_head=1):
        ...
        self.fdfd_post=feed_forward(hidden_nums,output_nums)
        self.fdfd_pre=feed_forward(hidden_nums,input_nums)

    def __call__(self,x,dropout_rate=0.05):

        z_output=self.fdfd_pre(x)

        z_output=tf.nn.dropout(z_output,dropout_rate)#0.2dropout
        #print(z_output,"fdfd encoder")
        z_output=add(x,z_output)
        #print(z_output,"fdfd add encoder")
        z_output_fdfd_pre=layer_norm(z_output)
        ...
        z_output,K,V=self.self_atten(z_output_fdfd_pre)

        z_output=tf.nn.dropout(z_output,dropout_rate)#0.2dropout
        #print(z_output.shape,"z_output.shape encoder")
        #print(z_output,"1z_output.shape encoder")
        z_output=add(z_output,z_output_fdfd_pre)
        ...
        z_output=self.fdfd_post(z_output_self_attentiont)

        z_output=tf.nn.dropout(z_output,dropout_rate)#0.2dropout
        #print(z_output,"fdfd encoder")
        z_output=add(z_output_self_attentiont,z_output)
        #print(z_output,"fdfd add encoder")
        z_output_fdfd_post=layer_norm(z_output)
        #print(z_output_fdfd,"z_output_fdfd")
        
        
        return z_output_fdfd_post,K,V
    def get_params(self):
        params=[]

        params.extend(self.self_atten.get_params())
        params.extend(self.fdfd_post.get_params())
        params.extend(self.fdfd_pre.get_params())
